"""Testing the calculation of the time-varying backup CBF"""
...
def main():
    ...
    parser.add_argument(
        "--env_config",
        type=str,
        default=os.path.join(_SCRIPT_DIR, "configs/env.config"),
    )
    parser.add_argument(
        "--policy_config",
        type=str,
        default=os.path.join(_SCRIPT_DIR, "configs/policy.config"),
    )
    ...
    device = torch.device("cuda:0" if torch.cuda.is_available() and args.gpu else "mps")
